	...
	const char*		lpLibFileName)
{
	...
	hModule = dlopen(lpLibFileName, RTLD_LAZY);
	if (!hModule)
		return NULL;
...
int EULoad()
{
	...
	s_Iface.SServerClientResetOperation =
		(PEU_SSERVER_CLIENT_RESET_OPERATION)
		GetProcAddress(s_hLibrary, "EUSServerClientResetOperation");

	s_Iface.DevCtxGenerateIDCardISPrivateKey =
		(PEU_DEV_CTX_GENERATE_IDCARD_IS_PRIVATE_KEY)
		GetProcAddress(s_hLibrary, "EUDevCtxGenerateIDCardISPrivateKey");
	...
	s_Iface.SServerClientResetOperation =
		EUSServerClientResetOperation;

	s_Iface.DevCtxGenerateIDCardISPrivateKey =
		EUDevCtxGenerateIDCardISPrivateKey;
